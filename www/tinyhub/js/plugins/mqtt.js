...
define(['css!../../styles/mqtt'], function () {
	...
	function MqttSwitch(props, broadcast) {
		this.htmlTemplate = '<label class="form-switch mqtt-switch"><input type="checkbox" id="{0}"/><i class="form-icon"></i>{1}</label>';
		this.props = props;
	...
	MqttButtons.prototype = {
		render: function() {
			return this.htmlTemplate.render(this.props.id, this.props.name, this.props.buttons[0].text);
		},
		...
		changed: function(value) {
			if (this.props.buttons[0].messageMap && this.props.buttons[0].messageMap[value])
				value = this.props.buttons[0].messageMap[value];
			this.dom.innerHTML = value;
		}
	};
